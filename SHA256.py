...
#const
CONST_K = [ '0x428a2f98', '0x71374491', '0xb5c0fbcf', '0xe9b5dba5', '0x3956c25b', '0x59f111f1', '0x923f82a4',
...
hash_value = ['0x6a09e667', '0xbb67ae85', '0x3c6ef372', '0xa54ff53a', '0x510e527f', '0x9b05688c', '0x1f83d9ab', '0x5be0cd19']
...
def addZero(bits, length=8, endian='LE'):
    l = len(bits)
    if endian == 'LE':
...
def preprocessMessage(message):
    ...
    if length < 448:
        bits.append(1)
        bits = addZero(bits, 448, 'LE')
        bits = bits + message_len
        return [bits]
    elif length == 448:
        bits.append(1)
        bits = addZero(bits, 1024, 'LE')
        bits[-64:] = message_len
...
def initializer(values):
    ...
    for binary in binaries:
        ...
        for b in binary:
            word.append(int(b))
        words.append(addZero(word, 32, 'BE'))
    return words
...
def sha256(message): 
    k = initializer(CONST_K)
    h0, h1, h2, h3, h4, h5, h6, h7 = initializer(hash_value)
